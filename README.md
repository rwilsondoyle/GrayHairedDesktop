...
**My Desktop** is a native desktop launch-page application for Zorin OS. It
displays a selected **Desktop Website** and provides easy access to favorite
shortcuts while ordinary links open in the system's default browser.

**My Desktop** is the owner-approved public/display product name.
GrayHairedDesktop remains the GitHub repository, Python package, and internal
compatibility identity; existing settings, paths, application IDs, and the
`grayhaired-desktop` command remain stable.
...
[`docs/RELEASE_READINESS.md`](docs/RELEASE_READINESS.md), the required physical
regression checks, and a separate final version-bump/release decision
remain. The public-name decision is complete.
