...
- Added eight compact, content-sized graphical launcher entries with local symbols and readable sample labels, leaving most vertical space for the Desktop Website.
- Established a reusable Favorite model containing a title, website address, and icon placeholder.
- Added seven temporary external sample destinations for manual testing without presenting them as final defaults; **Add Favorite** retains its future-update message.
- Preserved all Alpha 0.6 desktop environment behavior.
- Deferred persistence and all customization, editing, deleting, dialog, drag-and-drop, remote-artwork, and context-menu functionality.
