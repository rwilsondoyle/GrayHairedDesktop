...
**My Desktop** is the public display name. GrayHairedDesktop remains the repository and Python package identity. The `grayhaired-desktop` launcher and desktop filenames, `tech.grayhaired.GrayHairedDesktop` application ID, single-instance identity, existing GrayHaired Desktop QSettings identity, and application data/autostart paths remain unchanged for compatibility. GrayHaired Tech and Ron Doyle remain appropriate project attribution.
